package com.example.webflux.backpressure.strategy.bufferDropLatest;
	...
	public static void main(String[] args) {
		Flux
				...
				.onBackpressureBuffer(2, // 버퍼의 최대 크기를 2로 지정. 버퍼에 2개의 데이터가 들어가면 가득 참
						...
						BufferOverflowStrategy.DROP_LATEST) // 버퍼가 가득 찼을 때, 버퍼안에 들어있는 데이터중에서 가장 최근의 데이터를 Drop하는 전략
				.doOnNext(data -> Logger.info("# emitted by Buffer: {}", data)) // 버퍼에서 emit되는 데이터를 처리
				.publishOn(Schedulers.parallel(), false, 1) // 병렬 스케줄러에서 데이터를 처리하도록 지정함
